...
impl<T> Serialize for EnhanceMap<T>
where
    T: Ord + Serialize,
{
...
impl<T> Serialize for EnhanceLevel<T>
where
    T: Ord + Serialize,
{
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
...
impl<'de, T> Deserialize<'de> for EnhanceMap<T>
where
...
impl<'de, T> Deserialize<'de> for EnhanceLevel<T>
where
    T: Ord + Deserialize<'de>,
{
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
...
impl<'de, T> Visitor<'de> for &mut EnhanceMap<T>
where
    T: Ord + Deserialize<'de>,
{
    type Value = ();
...
impl<'de, T> Visitor<'de> for EnhanceLevel<T>
where
    T: Ord + Deserialize<'de>,
{
        ...
        A: MapAccess<'de>,
    {
        while let Some(key) = map.next_key::<String>()? {
            match key.as_str() {
                ...
                _ => Err(Error::unknown_field(&key, &["relative_rate", "absolute_rate", "broken_rate", "enhance_cost"]))?,
            }
        }
        Ok(self)
    }
